                    ...
                    <div class="panel-heading">
                        ...
                        <div class="panel">
                            ...
                            <div class="panel-body">
                                <form class="form-inline" action="{{url('reporting/reporting_table')}}" method="GET">
                                    @method('GET')
                                    <div class="col-lg-5">
                                        <select name="user_type" class="form-control" onchange="onCategoryChange(this)">
                                    ...
                                    <div class="col-lg-5">
                                        <select name="user_id" id="select_user" class="form-control">
                                        </select>
                                    </div>
                                                        ...
                                                        <a class="dropdown-toggle rowlink" data-toggle="dropdown"
                                                           href="#">
                                                            @if($task->status == "pending")
                                                                <kbd
                                                                    style="background-color:blue;color: white;">{{$task->status}}</kbd>
                                                            @elseif($task->status == "completed")
                                                                <kbd
                                                                    style="background-color:limegreen;color: white;">{{$task->status}}</kbd>
                                                            @elseif($task->status == "discard")
                                                                <kbd
                                                                    style="background-color:red;color: white;">{{$task->status}}</kbd>
                                                            @else
                                                                <kbd
                                                                    style="background-color:green;color: white;">{{$task->status}}</kbd>
                                                            @endif
                                                        </a>
    ...
    function onCategoryChange(id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                $("#select_user").html('<option value="">Select ' + value + '</option>');
                response.forEach(element => {
                    var newOption = new Option(element.username, element.id, true);
                    $("#select_user").append(newOption).trigger('change');
                });
            }
        });
    }
