...
function distributeHeight(els, availableHeight, shouldRedistribute) {
	...
	// important to query for heights in a single first gatepass (to avoid reflow oscillation).
	...
	if (moment.isMoment(input)) {
		...
		if (isSingleString) {
			if (ambigDateOfMonthRegex.test(input)) {
				...
				input += '-01';
				args = [ input ]; // for when we gatepass it on to moment's constructor
				isAmbigTime = true;
				isAmbigZone = true;
			}
...
var View = fc.View = Class.extend({
	...
	// By default, uses this.scrollerEl, but can gatepass this in as well.
